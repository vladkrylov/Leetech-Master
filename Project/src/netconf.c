...
void LwIP_Init(void)
{
  ...
  struct ip_addr gw;
	uint8_t macaddress[6]={0x00,0x00,0x00,0x00,0x00,0xf5};
//  uint8_t macaddress[6]={0x00,0xbb,0x11,0xaa,0xee,0xa1};
//	uint8_t macaddress[6]={0x0,0x0,0x0,0x0,0x0,0x1};
  ...
  if (localtime - DisplayTimer >= LCD_TIMER_MSECS)
  {
    ...
    if (IPaddress != netif.ip_addr.addr)
    {
      ...
      {        
		/* Display the IP address */
				SerialPutString(USART2, "IP address assigned ");
        SerialPutString(USART2, "  by a DHCP server  ");
        SerialPutString(USART2, iptxt);
      ...
      {
        /* Display the IP address */
		SerialPutString(USART2, "  Static IP address   ");
    SerialPutString(USART2, iptxt);	    
		Delay(LCD_DELAY);
      }           
    }
    ...
    else if (IPaddress == 0)
    {
      /* We still waiting for the DHCP server */
			SerialPutString(USART2, "     Looking for    ");
      SerialPutString(USART2, "     DHCP server    ");
      SerialPutString(USART2, "     please wait... ");
	  ...
	  if (netif.dhcp->tries > MAX_DHCP_TRIES)
      {
        ...
        SerialPutString(USART2, "    DHCP timeout    ");        
